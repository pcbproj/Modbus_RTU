...
void modbus_timers(void){
	if(Enable15) timer15++;
	if(timer15 < DELAY_1_5_BYTE_US) timer15_done_flag = 0;
	else timer15_done_flag = 1;
	
	if(Enable35) timer35++;
	if(timer35 < DELAY_3_5_BYTE_US) timer35_done_flag = 0;
...
//TODO: modbus_byte_timer нужно увеличивать в SysTickTimer прерывании
uint8_t ModbusReceiveByte(uint8_t usart_number, uint8_t *rx_byte){
	...
	case 1:
		while(!(USART1->SR & USART_SR_RXNE)){
			if(timer15_done_flag) return ERROR_PACK_LEN;
		}	
		*rx_byte = USART1 -> DR;
		break;
...
uint8_t RequestReceive(uint8_t rx_array[], uint8_t *rx_array_len){
	...
	ModbusReceiveByte(MODBUS_USART, &rx_byte);	// receive first byte = Address
